...
import sys, os, tempfile, rtf2xml.copy, rtf2xml.get_char_map
class Hex2Utf8:
    """
    ...
    def __initiate_values(self):
        ...
        # modules
        self.__char_file = os.path.join(os.path.dirname(rtf2xml.get_char_map.__file__), 'char_set.py')
        char_map_obj =  rtf2xml.get_char_map.GetCharMap(
                char_file = self.__char_file,
                bug_handler = self.__bug_handler,
                )
        up_128_dict = char_map_obj.get_char_map(map=self.__default_char_map)
        keys = up_128_dict.keys()
